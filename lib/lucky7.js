...
function pickFromPool(pool, stem, branch, usedSet) {
  const anchor = (stem * 7 + branch * 3) % pool.length;
...
export function computeUser5(dob, sex, birthTime) {
  ...
  for (const p of pillars) {
    ...
    const n = pickFromPool(RES_POOLS[elem], p.stem, p.branch, used);
  ...
  if (user5.length < 5) {
    ...
    const n = pickFromPool(RES_POOLS[branchElem], dayBranch, dayStem, used);
    if (n !== null) user5.push(n);
  }
...
export function computeHot1(hotPool, draw_date) {
  ...
  const inActive   = hotPool.filter(n => activePool.includes(n));
  const candidates = inActive.length > 0 ? inActive : [...hotPool];
